...
![应用方更新缓存a](/images/Redis/应用方更新缓存a.jpg)
...
由于缓存层承载着大量请求，有效地保护了存储层，但是如果缓存层由于某些原因不可用或者大量缓存由于超时时间相同在同一时间段失效，大量请求直接到达存储层，存储层压力过大导致系统雪崩。
...
![缓存击穿_分布式互斥锁](/images/Redis/缓存击穿_分布式互斥锁.jpg)
...
![缓存击穿_永不过期](/images/Redis/缓存击穿_永不过期.jpg)
